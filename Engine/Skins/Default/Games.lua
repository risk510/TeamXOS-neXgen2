-- Variables
AppNumber = GetMenuItems( GAME_LIST )
MainSelection = 0
ItemOffset = 0
FontAlpha = 255
...
for i=0, AppNumber-1 do
	TextureLoad("GAMETXR" .. i, GetItemIconPath(GAME_LIST,i))
end
...
function Render()
	...
	-- Draw Background
	DrawTexturedQuad( "Background_Title", 0, 640, 0, 480 )
	...
	for itemp = 0, AppNumber-1 do
		...
		if (FontPosY < 200) or (FontPosY > 420) then
			FontAlpha = 0
		else
			FontAlpha = 255
		end
		...
		if itemp == MainSelection then
			...
			DrawTexturedQuad( "GAMETXR" .. itemp, 412, 509, 75, 172)
		else
			FontColour( FontAlpha, 255, 255, 255 )
		end

		CurrentApp = GetMenuItemName( GAME_LIST, itemp )
...
function FrameMove()
		...
		if XBOX_PAD_A_BUTTON or XBOX_IR_SELECT then
			LaunchXBE(GetItemFilename(GAME_LIST, MainSelection))
		end
