-- Variables
AppNumber = GetMenuItems( APP_LIST )
MainSelection = 0
ItemOffset = 0
FontAlpha = 255
...
for i=0, AppNumber-1 do
	TextureLoad("APPSTXR" .. i, GetItemIconPath(APP_LIST,i))
end
...
function Render()
	...
	for itemp = 0, AppNumber-1 do
		...
		if itemp == MainSelection then
			...
			DrawTexturedQuad( "APPSTXR" .. itemp, 412, 509, 75, 172)
		else
			FontColour( FontAlpha, 255, 255, 255 )
		end


		CurrentApp = GetMenuItemName( APP_LIST, itemp )
...
function FrameMove()
		...
		if XBOX_PAD_A_BUTTON or XBOX_IR_SELECT then
			LaunchXBE(GetItemFilename(APP_LIST, MainSelection))
		end
