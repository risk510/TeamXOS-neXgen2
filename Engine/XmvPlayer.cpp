...
//-----------------------------------------------------------------------------
// Name: Initialise()
// Desc: Sets The pDevice
//-----------------------------------------------------------------------------
void CXMVPlayer::Initialise(LPDIRECT3DDEVICE8 pDevice)
{
	m_pDevice = pDevice;
}
...
//-----------------------------------------------------------------------------
HRESULT CXMVPlayer::FinishOpeningFile( D3DFORMAT format, BOOL bAllocateTextures   )
{
    ...
    for ( int i = 0; i < XMVPLAYER_NUMTEXTURES; i++ )
    {
        ...
        {
            hr = m_pDevice->CreateTexture( m_VideoDesc.Width, m_VideoDesc.Height, 1, 0, format, 0, &m_pTextures[i] );
            if ( FAILED( hr ) )
            {
                Destroy();
                return hr;
...
//-----------------------------------------------------------------------------
HRESULT CXMVPlayer::OpenFile( const CHAR* lpFilename, D3DFORMAT format, BOOL bAllocateTextures   )
{
    HRESULT hr  = S_OK;

    if ( NULL == lpFilename || NULL == m_pDevice )
    {
        return E_FAIL;
    }
    ...
    hr = FinishOpeningFile( format, bAllocateTextures );
...
//-----------------------------------------------------------------------------
HRESULT CXMVPlayer::OpenFileForPackets( const CHAR* lpFilename, D3DFORMAT format, BOOL bAllocateTextures  )
{
    HRESULT hr = S_OK;
    ...
    hr = XMVDecoder_CreateDecoderForPackets( XMVFLAG_SYNC_ON_NEXT_VBLANK | XMVFLAG_FULL_LOOP, first4Kbytes, ( DWORD )&m_loadContext,
...
//-----------------------------------------------------------------------------
HRESULT CXMVPlayer::OpenMovieFromMemory( const CHAR* lpFilename, D3DFORMAT format, BOOL bAllocateTextures  )
{
    HRESULT hr = S_OK;
...
//-----------------------------------------------------------------------------
LPDIRECT3DTEXTURE8 CXMVPlayer::GetTexture()
{
...
//-----------------------------------------------------------------------------
LPDIRECT3DTEXTURE8 CXMVPlayer::GetOverlay()
{
    // You must pass bAllocateTextures==TRUE to Open if you're going to use GetTexture/AdvanceFrame.
    assert( m_pTextures[0] );
    ...
    if ( pSubmittedTexture )
    {
        ...
        if ( m_pDevice->GetOverlayUpdateStatus() )
        {
...
//-----------------------------------------------------------------------------
void CXMVPlayer::EnableOverlays()
{
	m_pDevice->EnableOverlay( TRUE );
