...
void GetXboxNick()
{
	...
	{
		/* Nick Found now convert it to char*/
		// WCHAR to char Conversion
		 sprintf(szBuffer, "%ls", wszNickname );
		 XFindClose( hHandle );
	}
	else
	{
		sprintf(szBuffer, "%ls", "nexgen" );
	}
...
void DrawTexturedQuad(LPDIRECT3DTEXTURE8 pTexture, int iLeft, int iRight, int iTop, int iBottom, int iDestLeft, int iDestRight, int iDestTop, int iDestBottom)
{
	...
	D3DDevice::SetVertexData2f(D3DVSDE_TEXCOORD0, (float)iLeft, (float)iBottom);
	D3DDevice::SetVertexData4f(D3DVSDE_VERTEX, (float)iDestLeft, (float)iDestBottom, 0.0f, 1.0f);

	D3DDevice::SetVertexData2f(D3DVSDE_TEXCOORD0, (float)iLeft, (float)iTop);
	D3DDevice::SetVertexData4f(D3DVSDE_VERTEX, (float)iDestLeft, (float)iDestTop, 0.0f, 1.0f);

	D3DDevice::SetVertexData2f(D3DVSDE_TEXCOORD0, (float)iRight, (float)iTop);
	D3DDevice::SetVertexData4f(D3DVSDE_VERTEX, (float)iDestRight, (float)iDestTop, 0.0f, 1.0f);

	D3DDevice::SetVertexData2f(D3DVSDE_TEXCOORD0, (float)iRight, (float)iBottom);
	D3DDevice::SetVertexData4f(D3DVSDE_VERTEX, (float)iDestRight, (float)iDestBottom, 0.0f, 1.0f);
	D3DDevice::End();
}
...
string XGetDriveSize(char Drive)
{
	...
	if ( GetDiskFreeSpaceEx(szDirectory, &u64Free, &u64Total, &u64FreeTotal) )
	{
		...
		sReturn = XMakeNiceNumber(u64TotalBytes, _T('.'));
	}
...
string XGetDriveFree(char Drive)
{
	...
	if ( GetDiskFreeSpaceEx(szDirectory, &u64Free, &u64Total, &u64FreeTotal) )
	{
		ULONGLONG u64FreeTotalBytes = u64FreeTotal.QuadPart;

		sReturn = XMakeNiceNumber(u64FreeTotalBytes, _T('.'));
	}
...
string XMakeNiceNumber(ULONGLONG &u64Value, TCHAR tcComma)
{
	...
	if ( u64Trillions )
	{
		sprintf(szBuffer, "%I64d%c%03I64d%s", u64Trillions, tcComma, u64Billions, _T(" TB"));
	}

	else if ( u64Billions )
	{
		sprintf(szBuffer, "%I64d%c%03I64d%s", u64Billions, tcComma, u64Millions, _T(" GB"));
	}

	else if ( u64Millions )
	{
		sprintf(szBuffer, "%I64d%c%03I64d%s", u64Millions, tcComma, u64Thousands, _T(" MB"));
	}

	else if ( u64Thousands )
	{
		sprintf(szBuffer, "%I64d%c%03I64d%s", u64Thousands, tcComma, u64Remainder, _T(" KB"));
	}

	// Bytes
	else
	{
		sprintf(szBuffer, "%I64d%s", u64Remainder, _T(" bytes"));
	}
...
VOID XBUtil_DrawCustomRect( FLOAT fTLX, FLOAT fTLY, FLOAT fTRX, FLOAT fTRY, FLOAT fBLX, FLOAT fBLY,
							FLOAT fBRX, FLOAT fBRY, DWORD dwFillColor, DWORD dwOutlineColor, int iOutlineSize )
{
    D3DXVECTOR4 v[4];
    v[0] = D3DXVECTOR4( fTLX, fTLY, 0.0f, 1.0f );
    v[1] = D3DXVECTOR4( fTRX, fTRY, 0.0f, 1.0f );
    v[2] = D3DXVECTOR4( fBRX, fBRY, 0.0f, 1.0f );
    v[3] = D3DXVECTOR4( fBLX, fBRY, 0.0f, 1.0f );
